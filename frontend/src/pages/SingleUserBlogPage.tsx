import { Stack, Paper, Typography, Button, Chip, Box, Alert} from "@mui/material"
                ...
                <Stack spacing={3}>
                    ...
                    >
                        ...
                        <Stack gap={1}>
                            {blog?.blogAuthor ? (
                                <>
                                    <Typography color="primary" variant="subtitle1" gutterBottom> Author Details: </Typography>
                                    <Chip color="secondary" icon={<Person/> }>{blog?.blogAuthor?.userName}</Chip>
                                    <Chip color="primary" icon={<Email/>}>{blog?.blogAuthour?.email}</Chip>
                                </>
                            ) : (
                            <>
                                <Button
                    ...
                    {deleteForm && (
                        <Box component={"section"} bgcolor={"#ff000"} border={"1px solid red"} 
                        p={2} borderRadius={1} width={{xs: "100%", md:"70%"}}>
                            <Typography variant="h6" gutterBottom color="error" >
                                Are you sure you want to delete this Blog?
                            </Typography>
                            <Stack direction={"row"} gap={2}>
                                <Button sx={{textTransform: "none"}} variant="contained" onClick={handleCancelDelete}>Cancel</Button>
                                <Button sx={{textTransform: "none"}} variant="outlined" color="error" onClick={handleDeleteBlog}>Yes, I'm Sure</Button>
                            </Stack>
                        </Box>
                    )}
                </Stack>
